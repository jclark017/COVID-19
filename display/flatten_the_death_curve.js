...
var margin = {top: 20, right: 15, bottom: 25, left: 50};

function SDLineChart () {
    // parse the date / time
    ...
    d3.csv('../sql/SumByCountryDateAnalysis.csv', function (error, dataProto) {
        ...
        ///////////////////////////////////////
        // Paint the Second Derivative Chart //
        ///////////////////////////////////////
        // set the ranges
        ...
        y.domain([-d3.max(data, function(d) {return d.ConfirmedSDSMA4 }),d3.max(data, function(d) {return d.ConfirmedSDSMA4 })]);

        
        var valueline = d3.line()
        .x(function(d) { return x(d.Last_Update); })
        .y(function(d) { return y(d.ConfirmedSDSMA4);  })
        .curve(d3.curveMonotoneX);

        /* US Data */
        var dataUS = data.filter(function(d){return d.Country_Region=="US"})
        ...
        /* UK Data */
        var dataUK = data.filter(function(d){return d.Country_Region=="UK" || d.Country_Region=="United Kingdom"})
        ...
        /* Spain Data */
        var dataSpain = data.filter(function(d){return d.Country_Region=="Spain"})

        svg.append("path")
        .data([dataSpain]) 
        .attr("class", "lineSpain")  
        .attr("d", valueline); 

        /* Italy Data */
        var dataItaly = data.filter(function(d){return d.Country_Region=="Italy"})

        svg.append("path")
        .data([dataItaly]) 
        .attr("class", "lineItaly")  
        .attr("d", valueline); 
...
function DailyChangeChart () {
    ...
    d3.csv('../sql/SumByCountryDateAnalysis.csv', function (error, dataProto) {
        ...
        var y = d3.scaleLinear().range([height, 0]);
        y.domain([d3.min(data, function(d) {return d.ConfirmedDailyIncrease }),d3.max(data, function(d) {return d.ConfirmedDailyIncrease })]);

        
        var valueline = d3.line()
        .x(function(d) { return x(d.Last_Update); })
        .y(function(d) { return y(d.ConfirmedDailyIncrease);  })
        .curve(d3.curveMonotoneX);
...
function TotalChart () {
        ...
        d3.csv('../sql/SumByCountryDateAnalysis.csv', function (error, dataProto) {
            ...
            var y = d3.scaleLinear().range([height, 0]);
            y.domain([d3.min(data, function(d) {return d.Confirmed }),d3.max(data, function(d) {return d.Confirmed })]);
