...
var chartTypes = [
    ...
    {"name" :"Deaths" , "location": {"left": (3*cSpace) + (2*width), "top": 60 + (i*hSpace) + (i*height), "position":"absolute"}, "ybasis":"zero"},
    {"name" :"DeathsDailyIncrease" , "location": {"left": (4*cSpace) + (3*width), "top": 60 + (i*hSpace) + (i*height), "position":"absolute"}, "ybasis":"zero"},
    /*{"name" :"DeathsSDSMA4" , "location": {"left": (6*cSpace) + (5*width), "top": 60 + (i*hSpace) + (i*height), "position":"absolute"}, "ybasis":"function"},*/
    {"name" :"TodayPositiveTestRate" , "location": {"left": (5*cSpace) + (4*width), "top": 60 + (i*hSpace) + (i*height), "position":"absolute"}, "ybasis":"zero"},    
    {"name" :"hospitalizedCurrently" , "location": {"left": (6*cSpace) + (5*width), "top": 60 + (i*hSpace) + (i*height), "position":"absolute"}, "ybasis":"zero"},    
    {"name" :"onVentilatorCurrently" , "location": {"left": (7*cSpace) + (6*width), "top": 60 + (i*hSpace) + (i*height), "position":"absolute"}, "ybasis":"zero"}    
]
...
function LineChartState() {
  d3.json('https://covidtracking.com/api/v1/states/daily.json', function (error, dataProto) {
    d3.json('https://covidtracking.com/api/v1/states/info.json', function (error, stateProto) {
      d3.json('./statedata.json', function (error, statePop) {
      /*"date":20200630,
      "state":"AK",
      "positive":940,
      "negative":111245,
      ...
      */
      ...
      state.forEach(function(stateData, i) {
        // Create and paint each chart
        trimStateName = stateData.name.replaceAll(' ','');
        ...
        chartTypes.forEach(function(chartType) {
          ...
          var dataFiltered = data.filter(function(d){return d.state==stateData.state})
          //Set with Nan excluded for cleaner data
          var dataFilteredNan = data.filter(function(d){return d.state==stateData.state && d[chartType.name] })
